...
func (d DbConnNum) Register(cm iface.ComponentMeta) {
	cm.AddParameters(cons.STRING, cons.LOWER_H, "ip", "host", true,
		func(s string) error {
			if !utils.CheckIp(s) {
	...
	cm.AddParameters(cons.STRING, cons.LOWER_U, "username", "username", true, nil, "数据库用户名")
	cm.AddParameters(cons.STRING, cons.LOWER_W, "password", "password", true, nil, "数据库密码")
	cm.AddParameters(cons.STRING, cons.LOWER_D, "dbname", "dbname", true, nil, "数据库名称")
	cm.AddParameters(cons.STRING, cons.LOWER_S, "searchpath", "schema", true, nil, "数据库schema")
}
